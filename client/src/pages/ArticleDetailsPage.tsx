...
export default function ArticleDetailsPage() {
  ...
  useEffect(() => {
    if (id) loadArticleById(id);
  }, [id, loadArticleById]);

  const resolveImageUrl = (imageUrl?: string) => {
    if (!imageUrl) return DEFAULT_IMAGES.ARTICLE;
  ...
  const handleImageError = (e: React.SyntheticEvent<HTMLImageElement>) => {
    (e.target as HTMLImageElement).src = DEFAULT_IMAGES.ARTICLE;
  };
  ...
  if (!article)
    return (
      <>
        {backgroundElement}
        <ErrorMessage message="The article you're looking for doesn't exist or is not available." />
      </>
    );
    ...
    <>
      ...
      <div className="relative min-h-screen w-full px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        >
          ...
          <p className="text-base sm:text-lg text-[#567544] italic mb-6">
            {article.description}
          </p>

          <div
            className="prose prose-sm sm:prose-base max-w-none text-[#567544] leading-relaxed"
